...
│   (ESP32-S3)    │   (ON DEMAND)  │   Server    │
│   - Mesh Root   │                └─────────────┘
│   - GPRS Module │   ⏰ TIME SYNC 
│   - Deep Sleep  │   📊 DATA SEND 
│   - RTC Module  │                
└─────────────────┘                
         │  📡 SYNC BROADCAST       
    ┌────┴────┐                    
...
│ESP32-C3│ │ESP32-C3│               
│BME280 │ │BME280 │     ⏰ ALL WAKE UP 
│Battery│ │Battery│        AT SAME TIME
...
- ✅ **ALL nodes sleep** (including gateway)
- ✅ **Synchronized wake-up** at exact same time
- ✅ **GPRS on-demand only** (time sync + data upload)
- ✅ **RTC time keeping** during deep sleep
- ✅ **Coordinated mesh cycles** (wake → collect → send → sleep)
...
#### **TASK 3: Gateway - Force Root Role + RTC** ⭐ (MINIMAL CHANGE)
...
// 🆕 Initialize RTC for time keeping during sleep
esp_sleep_enable_timer_wakeup(300 * 1000000ULL); // 5 minutes default
ESP_LOGI(TAG, "GATEWAY: RTC configured for time synchronization");
```
...
#### **TASK 4: Sensor - Child Only Role + RTC** ⭐ (MINIMAL CHANGE)
...
// 🆕 Initialize RTC for synchronized wake-up
esp_sleep_enable_timer_wakeup(300 * 1000000ULL); // Default 5 minutes
ESP_LOGI(TAG, "SENSOR: RTC configured for synchronized operation");
```
